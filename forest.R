...
function(var = NULL, stat = NULL, lay = NULL, layID = NULL, pol = NULL, sour = NULL) {
  ...
  if (!is.null(pol)){
    ...
    ## Create a gdal-readable object from WKT and write # it would be nice to use sf objects instead
    ...
    if (sour == 'hansen_armonized'){
      ...
      ##Cut Colombia ARMONIZED layers with WKT geometry
      system.time(gdalwarp(srcfile = '/data/input/forest/hansen_ARM/tree_col.tif',
                           dstfile = treeTemp, 
                           crop_to_cutline = TRUE,
                           cutline = paste0(tmpF, '.shp'))) # 68
      
      system.time(gdalwarp(srcfile = '/data/input/forest/hansen_ARM/loss_col.tif', 
                           dstfile = lossTemp, 
                           crop_to_cutline = TRUE,
                           cutline = paste0(tmpF, '.shp')))
