...
```
Escribir el comando PSCP. Incluye la clave .ppk (con ruta si es necesario), el nombre del archivo que se quiere transferir, la dfirección IPv4 de la instancia y la ruta al directorio donde se va a almacenar el archivo:
```{bash, eval=FALSE}
...
En el directorio  ubuntu/home/plumber están almacenados los scripts y funciones necesarios para los procesos en el backend, así como los componentes para configurar y ejecutar Docker 

1. biotablero_bash.sh: Archivo Bash de configuración incial de la instancia.
2. main.R: R script de inicio de  BioTablero API
3. biotablero_api.R: API y funciones indicadoras.
...
7. init-mongo.js: crea al usuariuo de la base de datos Mongo.
...
### Instalar programas y configuar el sistema

Montar unidad de almacenamiento externo de 30GB. Esta nueva caropeta esta en The folder will be ´/data´
...
Instalar docker, docker-compose, 7zip y tree.
...
Ejecutar los comandos de Docker sin **sudo**. Es posible que sea necesario reiniciar la conexión con el hos . That's why some docker commands bellow will use ´sudo´ **Esto no lo entiendo**
...
Download docker-compose (ver si hay versión nueva)
```{bash, eval=FALSE}
sudo curl -L "https://github.com/docker/compose/releases/download/1.25.5/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
sudo chmod +x /usr/local/bin/docker-compose
```

### Descargar los datasets comprimidos

Los datasets se almacenan en la carpeta ´/data´.

#### Especies data sets

Biomodelos (Biomod.7z). 6113 species datasets from 1340097 locations (.RData files). **Como actualizamos esto?** 
...
This needs to be updated in the cloud server, so everything gets installed and loaded at once!!
...
**También ver si esto se puede / hay con que actualizarlo**
...
### Crear directorios y  descomprimir los datos.
